...
Current source code candidate: `128440f1332334d149c4bc4a215f2f12a0588e2a`; evidence commit: `53973cebc3c2599d862f552f4d22936ecd918034`. The fresh manual clone at `53973ce` passed lint/typecheck, **13 frontend unit tests**, production build, **57 direct tests in 13.10s**, GenVM check (3 lint checks + SDK validation), SDK typecheck and schema (25 methods: 14 views, 11 writes). Its Playwright process reported three cases passed but did not exit cleanly; GitHub Actions run [34851021590](https://github.com/Ifem1/apterra/actions/runs/34851021590) passed all workflow steps, including 3 browser smoke tests in 15.6s and the 57-test direct suite in 25.58s. Local GenVM commands emitted Windows `WinError 10013` on latest-release metadata lookup and then used the explicitly pinned RC5 cache. Exact source hashes are in [TOOLCHAIN](TOOLCHAIN.md). Contract SHA-256 remains `34b995a9b0b5b9b75bb2f29bb5793eae213171a1c26026e4cbd8d4ca903121eb`. Evidence prose was synchronized in a docs-only commit after these source checks; that commit does not alter runtime code. The candidate has no deployment proposal because full Phase 1 scope and end-to-end acceptance are not ready. There is no current deployment fee quote and no signature request.
...
| Contract lint, SDK validation, schema, typecheck | PASS in manual clone `53973ce` and GitHub run `34851021590`; pinned v0.6.0-rc5 bundle; 3 lint checks + SDK validation, 25-method schema and no SDK type errors |
| Direct test suite | PASS locally at `128440f`: 57 passed in 11.64s; manual clone `53973ce`: 57 passed in 13.10s; CI: 57 passed in 25.58s. All use pinned RC2 runner, GenVM RC5 |
