...
Manual clean-clone verification used `git clone --depth 1 --branch main https://github.com/Ifem1/apterra.git C:\Users\DELL\AppData\Local\Temp\apterra-verify-final-53973ce-20260914`, with clean HEAD `53973cebc3c2599d862f552f4d22936ecd918034`. `npm ci` installed 503 locked packages. The clone passed `npm run lint`, `npm run test:frontend` (**13/13**), and `npm run build`; the local Playwright run reported all three cases passed, but its Windows process did not exit cleanly and was interrupted. The green remote run at [34851021590](https://github.com/Ifem1/apterra/actions/runs/34851021590) on the same code passed all three Chromium cases in 15.6s. With `USERPROFILE=%TEMP%\apta`, `GENVM_PREBUILT_DIR` set to the pinned RC5 bundle, and `PYTHONIOENCODING=utf-8`, the clone passed `python -m pytest -q --tb=short` (**57 passed in 13.10s**). `genvm-lint check` passed 3 lint checks and SDK validation over 25 methods; SDK typecheck found no errors; schema extraction reported 25 methods (14 views, 11 writes, 0 constructor parameters). GenVM tools warned that GitHub metadata lookup was blocked by Windows `WinError 10013`, then completed using the explicitly pinned local v0.6.0-rc5 bundle. `npm audit --omit=dev` found **0 vulnerabilities**; full `npm audit` found five moderate findings via the pinned CLI's Vitest dependency and dockerode's nested uuid. `npm ci` also printed upstream deprecation notices. No dependency override was added because remediation would affect the pinned RC CLI. This verification covers a clean checkout; it is not live consensus proof.

GitHub Actions run [34850126835](https://github.com/Ifem1/apterra/actions/runs/34850126835) passed on clean checkout `128440f`: Studio Dev guard + ESLint + TypeScript, frontend tests **13/13**, Chromium smoke **3/3 in 15.2s**, production build, `genvm-lint check` (**3 lint checks + SDK validation**, 25 contract methods), `genvm-lint typecheck` (no errors), schema extraction (**25 methods: 14 views, 11 writes, 0 constructor parameters**), and direct suite **57 passed in 25.58s**. Run [34851021590](https://github.com/Ifem1/apterra/actions/runs/34851021590) passed the same workflow on evidence commit `53973ce`. The previous remote run failed because the timestamp-fixture shim referenced a Windows-only import; the corrected cross-platform shim is in `128440f`. The same source passed the direct suite locally on Windows: **57 passed in 11.64s**. Earlier clean-clone install reported five moderate development-only npm advisories; they remain in the current lockfile. None of the direct-mode tests substitutes for consensus execution.
