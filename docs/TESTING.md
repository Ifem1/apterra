...
At `a6216f8174091be775bfda9068fd4ee3ef2bbf6a`, `npm run lint` (including typecheck and Studio Dev-only guard), `npm run test:frontend` (8/8), and `npm run build` passed; the build emitted a Next.js plugin-detection warning even though explicit ESLint passed. Browser smoke on this commit passed 3/3 in 11.9s against a manually started dev server. The clean-clone rerun is pending. The prior source candidate `58491a821d17ace5719cca3a4452801348fc165e` passed `npm ci` (504 packages), all frontend checks, GenVM check/schema/typecheck, and direct suite 50/50; use `git clone --branch main https://github.com/Ifem1/apterra.git` for a clean verification after the latest pushed commit. Pin download requires `$env:PYTHONIOENCODING='utf-8'`; once cached, the linter still warns that Windows cannot resolve GitHub latest-runner metadata (`WinError 10013`) but uses RC5. `npm audit --omit=dev` could not reach the npm advisories endpoint in this network-restricted environment; no audit result is claimed. No local direct test substitutes for consensus execution.
