...
The current CI baseline passes **43/43 deterministic frontend tests**, **3/3 Chromium smoke cases**, production build, GenVM lint (**3/3 lint checks + SDK validation**), GenVM SDK typecheck with no errors, schema extraction (**25 methods: 14 views, 11 writes, 0 constructor parameters**), and the full direct Python suite at **63/63**. The Studio Dev-only guard scans 23 runtime/config files. Direct mode remains test execution with fixtures, not live consensus proof.

## Studio Dev and live flow

Only `studio-dev` / SDK `studioDevnet` / canonical RPC `https://studio-dev.genlayer.com/api` / chain 61997 (`0xf22d`) are allowed. Read `eth_chainId` immediately before every live deployment/write session and fail closed otherwise. The browser automatically follows each newly submitted transaction using the same hash through finality and canonical readback where mapped; manual decision/finality controls remain fallback. It must never blindly resubmit after timeout or tracking failure. Keep the deployment proposal paused until the owner has reviewed and approved the exact source, constructor, sender, fresh operation-specific fee quote, and state effect. Record finalized decision, execution result and canonical readback in the single [requirements/evidence ledger](REQUIREMENTS_MATRIX.md).

Do not run a new deployment or consequential wallet transaction without explicit owner wallet signing. The canonical Studio Dev deployment is `0xfDB7f4C28F157662133dF51B5a864BDa3F4B7060` on chain 61997 and the public frontend is live at https://apterra.vercel.app/; live v1 evidence and transaction boundaries are recorded in [LIVE_EVIDENCE](LIVE_EVIDENCE.md).
